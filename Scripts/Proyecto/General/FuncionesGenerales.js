var FuncionesGenerales = {
    ...
    LlamadaAjax: function (url, parametros, funcionSuccess) {
        return $.ajax({
            ...
            error: function (xmlHttpRequest, errorThrown) {
                FuncionesGenerales.OcultarCargando();
                if (xmlHttpRequest.status === 511) {
                    MensajesGenerales.MensajeAdvertenciaRedireccion('La sesión ha expirado.<br>Por favor inicie sesión nuevamente.', window.location.origin + '/FISC-PE/Login');
                } else {
    ...
    ValidaDocumentoCapturado: function (IdDocumentoCapturado) {
        return IdDocumentoCapturado === 'qJuIgGFiAEw=' ? '<button class="btn btn-sm btn-primary crear" id="btnCrear"><i class="fas fa-edit"></i></button>'
            : '<button class="btn btn-sm btn-primary imprimir" id="btnImprimir"><i class="fas fa-file-pdf"></i></button>';
    }
};